"""ONS: House price to residence-based earnings ratio — download and tidy."""
...
from ons_price_residence_earnings_ratio_config import (
    ...
    PRICE_RESIDENCE_EARNINGS_RATIO_HEADER_ROW,
)
...
def read_data_sheet(path: Path, sheet: str) -> pd.DataFrame:
    df = pd.read_excel(
        path,
        sheet_name=sheet,
        header=PRICE_RESIDENCE_EARNINGS_RATIO_HEADER_ROW,
        engine="openpyxl",
    )
...
def transform_workbook(
    ...
    edition_key: str,
    *,
    file_prefix: str = "ons_price_residence_earnings_ratio",
...
def _edition_from_key(key: str) -> EpcEdition:
    if key not in PRICE_RESIDENCE_EARNINGS_RATIO_EDITIONS:
        raise SystemExit(
            f"Unknown edition {key!r}. Choose one of: {', '.join(sorted(PRICE_RESIDENCE_EARNINGS_RATIO_EDITIONS))}."
        )
    return PRICE_RESIDENCE_EARNINGS_RATIO_EDITIONS[key]


def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="ONS house price to residence-based earnings ratio (England and Wales).",
    )
    p.add_argument(
        "--edition",
        default="current",
        help="Edition key from ons_price_residence_earnings_ratio_config (default: current).",
    )
    p.add_argument(
        "--raw-dir",
        type=Path,
