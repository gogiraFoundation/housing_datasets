"""Streamlit: LA choropleth — house building, median price, affordability, or regional EPC proxy."""
...
def main() -> None:
    ...
    st.title("Map: local authority choropleth")
    st.caption(
        "GeoJSON boundaries under `data/geo/` (see `data/geo/README.md`). "
        "Run `python scripts/download_lad_boundaries.py` for full UK LAD polygons (WGS84)."
    )
    st.divider()
    ...
    for feat in feats:
        ...
        props["tooltip_raw"] = (
            _fmt_num(raw_clean, per_1000=per_eff) if metric == "house_building" else "—"
        )
    ...
    tooltip = folium.GeoJsonTooltip(
        fields=["tooltip_name", "tooltip_code", "tooltip_value", "tooltip_raw", "tooltip_fy"],
        aliases=["Local authority", "Code", "Mapped value", "Raw / secondary", "Period / note"],
        sticky=True,
        localize=False,
    )
    ...
    with col_a:
        ...
        show = top[["la_name", "lad_code", "region", "value"]].rename(
            columns={
    ...
    with col_b:
        ...
        show_b = bottom[["la_name", "lad_code", "region", "value"]].rename(
            columns={
                "la_name": "Local authority",
                "lad_code": "Code",
                "region": "Region / nation",
