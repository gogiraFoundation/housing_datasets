"""Streamlit: ONS house price (newly built dwellings) to workplace-based earnings ratio (England and Wales)."""
...
from ons_price_earnings_ratio_config import PRICE_EARNINGS_RATIO_DATA_SHEETS
...
_TABLE_HELP = {
    "1a": "1a — Median new-build house price (£), country / region",
    "1b": "1b — Median workplace earnings (£), country / region",
    "1c": "1c — Median price / median earnings ratio, country / region",
    "2a": "2a — Lower quartile new-build house price (£), country / region",
    "2b": "2b — Lower quartile workplace earnings (£), country / region",
    "2c": "2c — Lower quartile price / earnings ratio, country / region",
    "3a": "3a — Median new-build house price (£), county (England)",
    "3b": "3b — Median workplace earnings (£), county (England)",
    "3c": "3c — Median price / median earnings ratio, county (England)",
    "4a": "4a — Lower quartile new-build house price (£), county (England)",
    "4b": "4b — Lower quartile workplace earnings (£), county (England)",
    "4c": "4c — Lower quartile price / earnings ratio, county (England)",
    "5a": "5a — Median new-build house price (£), local authority",
    "5b": "5b — Median workplace earnings (£), local authority",
    "5c": "5c — Median price / median earnings ratio, local authority",
    "6a": "6a — Lower quartile new-build house price (£), local authority",
    "6b": "6b — Lower quartile workplace earnings (£), local authority",
    "6c": "6c — Lower quartile price / earnings ratio, local authority",
}


def _expected_parquet_path(edition: str, table: str) -> Path:
    return PROCESSED_DIR / f"ons_price_newbuild_workplace_earnings_ratio_{edition}_{table}_tidy.parquet"
...
def _y_title(component: str) -> str:
    if component == "house_price":
        return "New-build house price (£)"
    if component == "earnings":
        return "Gross annual workplace-based earnings (£)"
    return "Ratio (new-build house price ÷ earnings)"


def main() -> None:
    st.set_page_config(page_title="New build / workplace earnings", layout="wide")
    st.title("House price (newly built dwellings) to workplace-based earnings ratio")
    st.caption(
        "ONS affordability tables for **newly built dwellings** only: **median** and **lower quartile** "
        "house prices and **workplace-based** earnings, and **ratios**, for country / region, county (England), "
        "and local authority. Run `python ons_price_newbuild_workplace_earnings_ratio_etl.py --edition current`."
    )
    st.divider()
    ...
    edition = st.sidebar.selectbox(
        ...
        format_func=lambda k: f"{k} ({NEWBUILD_WORKPLACE_PRICE_EARNINGS_EDITIONS[k].label})",
        index=0,
    )
    table = st.sidebar.selectbox(
        "Table",
        options=list(PRICE_EARNINGS_RATIO_DATA_SHEETS),
    ...
    if not path.is_file():
        ...
        st.code(
            "python ons_price_newbuild_workplace_earnings_ratio_etl.py --edition "
            + edition
            + "\n# or: python ons_price_newbuild_workplace_earnings_ratio_etl.py --transform-only -i workbook.xlsx --edition "
            + edition,
            language="bash",
        )
        return
    ...
    st.download_button(
        ...
        data=csv_bytes,
        file_name=f"newbuild_workplace_price_earnings_ratio_{edition}_{table}_filtered.csv",
        mime="text/csv",
    )
