"""Streamlit: ONS house price to workplace-based earnings ratio (England and Wales)."""
...
from ons_price_earnings_ratio_config import DATASET_PAGE, PRICE_EARNINGS_RATIO_DATA_SHEETS, PRICE_EARNINGS_RATIO_EDITIONS
...
def _expected_parquet_path(edition: str, table: str) -> Path:
    return PROCESSED_DIR / f"ons_price_earnings_ratio_{edition}_{table}_tidy.parquet"
...
def main() -> None:
    st.set_page_config(page_title="Price / earnings ratio", layout="wide")
    st.title("House price to workplace-based earnings ratio")
    st.caption(
        ...
        "Run `python ons_price_earnings_ratio_etl.py --edition current`."
    )
    st.divider()
    ...
    if not path.is_file():
        ...
        st.code(
            "python ons_price_earnings_ratio_etl.py --edition "
            + edition
            + "\n# or: python ons_price_earnings_ratio_etl.py --transform-only -i workbook.xlsx --edition "
            + edition,
            language="bash",
        )
        return
    ...
    st.download_button(
        ...
        data=csv_bytes,
        file_name=f"price_earnings_ratio_{edition}_{table}_filtered.csv",
        mime="text/csv",
    )
