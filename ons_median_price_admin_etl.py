...
_GEO_LEVEL = {"1": "region", "2": "local_authority", "3": "county_unitary", "4": "combined_authority"}
...
def _meta_for_sheet(sheet: str) -> tuple[str, str]:
    ...
    return _GEO_LEVEL[row], _PROPERTY_BAND[col]
...
def read_data_sheet(path: Path, sheet: str) -> pd.DataFrame:
    df = pd.read_excel(
        path,
        sheet_name=sheet,
        header=MEDIAN_PRICE_ADMIN_HEADER_ROW,
        engine="openpyxl",
    )
...
def transform_sheet(df: pd.DataFrame, sheet: str, *, dwelling_class: str) -> pd.DataFrame:
    geo_level, property_band = _meta_for_sheet(sheet)
    ...
    tidy = df.melt(id_vars=id_vars, var_name="period_label", value_name="median_price_gbp")
    ...
    tidy["dwelling_class"] = dwelling_class
    geo_cols = [ren[c] for c in id_vars]
    return tidy[
        ["table_id", "dwelling_class", "geography_level", "property_band", *geo_cols, "period_label", "median_price_gbp"]
    ]
...
def transform_workbook(
    ...
    *,
    ...
    for sheet in MEDIAN_PRICE_ADMIN_DATA_SHEETS:
        ...
        tidy = transform_sheet(raw, sheet, dwelling_class=dwelling_class)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        help="Edition key from ons_median_price_admin_config (default: yearendingseptember2025).",
    )
    p.add_argument(
        "--raw-dir",
        type=Path,
...
def main() -> None:
    ...
    edition = _edition_from_key(args.dataset, args.edition)
    ...
    if args.dataset == "existing":
        ...
        file_prefix = "ons_median_price_new_admin"
        dwelling_class = "new_build"
